...
def get_initial_state(params: ModelParameters) -> np.ndarray:
    ...
    # Lymphocyte distribution based on Westermann & Pabst (1992):
    # 2% in blood, 60% in tissues (spleen), 38% in lymph (node + lymph fluid)
    # Reference: Li et al. (2022) Supplementary Material (CPT_2729_2022-0235-s01)
    ...
    # Total body pools given "2% in blood" assumption
    TOTAL_T = T_blood_baseline / 0.02
    TOTAL_B = B_blood_baseline / 0.02

    # Distribution fractions: 2% blood, 60% spleen, 38% (node + lymph)
    frac_blood  = 0.02
    frac_spleen = 0.60
    frac_rest   = 0.38
