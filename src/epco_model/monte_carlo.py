...
# Disable threading in NumPy/SciPy BLAS libraries to avoid conflicts with multiprocessing
...
def _simulate_one_patient(
    ...
    # Thread-safe containers for result and exception
    result = [None]
    exception = [None]
...
def run_virtual_population_parallel(
    ...
    backend: str = "loky", # "sequential" is stable, "loky"/"threading" are experimental
    ...
    print(f"[Main] Starting population: n_patients={n_patients}, backend={backend}, n_jobs={n_jobs}")
